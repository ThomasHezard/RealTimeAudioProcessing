...
JNIEXPORT void JNICALL
        Java_com_google_sample_echo_MainActivity_setParam(JNIEnv *env, jclass type, jint id, jfloat value);
}
...
Java_com_google_sample_echo_MainActivity_setParam(JNIEnv *env, jclass type, jint id, jfloat value) {
    ...
    if (engine.player_ != nullptr) {
        engine.player_->processor()->set_param(paramID, paramValue);
    }

/*    LOGE("jm/debug Java_com_google_sample_echo_MainActivity_setParam, "
         "%d %f",
         paramID, paramValue);*/
